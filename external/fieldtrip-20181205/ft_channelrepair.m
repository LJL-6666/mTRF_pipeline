...
if any(isnan(sens.chanpos(:)))
  ft_error(['The channel positions contain NaNs; this prohibits correct ' ...
      'behavior of the function. Please replace the input channel definition with one that contains valid channel positions']);
end

if ismeg && ~any(strcmp(ft_senstype(sens), {'ctf151', 'ctf275', 'bti148', ...
        'bti248', 'babysquid74'}))
  % MEG systems with only magnetometers or axial gradiometers are easy, planar systems are not
  ft_warning(['be careful when using "%s" - mixing of sensor types ' ...
      '(e.g. magnetometers and gradiometers) can lead to wrong data. ' ...
      'Check your neighbour-structure thoroughly'], ft_senstype(sens));
end
...
if ~isempty(cfg.missingchannel) && strcmp(cfg.method, 'weighted')
  ft_warning(['Reconstructing missing channels using the weighted neighbour ' ...
    ...
    for k=badindx'
      ...
      if strcmp(cfg.method, 'weighted')
        distance = sqrt(sum((sens.chanpos(goodsensindx, :) - ...
            repmat(sens.chanpos(badsensindx, :), numel(goodsensindx), 1)).^2, 2));
  ...
  if ~isempty(cfg.missingchannel)
    ...
    for k=missingindx'
      ...
      if isempty(goodsensindx)
        fprintf(['\tcannot reconstruct channel - no neighbours in the ' ...
            'original data or in the sensor position\n']);
        unable = [unable k];
      else
...
elseif strcmp(cfg.method, 'spline') || strcmp(cfg.method, 'slap')
  if ~isempty(cfg.badchannel) || ~isempty(cfg.missingchannel)
    fprintf(['Spherical spline and surface Laplacian interpolation will ' ...
        'treat bad and missing channels the same. Missing channels will ' ...
        'be concatenated at the end of your data structure.\n']);
  end

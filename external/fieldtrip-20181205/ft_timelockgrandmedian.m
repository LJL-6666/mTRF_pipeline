function [grandavg] = ft_timelockgrandmedian(cfg, varargin)
...
if strcmp(cfg.keepindividual, 'yes')
  ...
  ResultDOF      = reshape(sum(avgdof, 1,'omitnan'), datsiz);
  grandavg.avg   = reshape(median(avgmat, 1,'omitnan'), datsiz); % computes both means (plain and weighted)
  % Nchan x Nsamples, skips the singleton
  % if strcmp(cfg.method, 'across')
  ResultVar      = reshape(sum(avgvar,1,'omitnan'), datsiz)-reshape(sum(avgmat,1,'omitnan'), datsiz).^2./ResultDOF;
  % else  % cfg.method = 'within'
